...
Setting the gradient to zero gives a clean closed-form solution:

    beta = (X^T X + alpha * I')^(-1) X^T y

where I' is the identity matrix with a zero in the (0, 0) slot so the
intercept term is NOT penalised — this matches the convention used by
scikit-learn's Ridge.

Unlike Lasso, Ridge shrinks every coefficient continuously toward zero but
never exactly to zero, so it does not perform feature selection. It is
particularly useful when features are highly correlated, where it
distributes coefficient weight more evenly across them.
"""
...
class RidgeRegression:
    ...
    def build_model(self):
        ...
        # np.linalg.solve is more numerically stable than computing the inverse explicitly.
